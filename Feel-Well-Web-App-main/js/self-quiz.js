...
// Determine the question set based on the class of an element
const selector=document.querySelector('.wid-main');
var set;
var flag;

// Check if the class 'depression' is present
if(selector.classList.contains('depression')){
   set=questions2;
   flag=1; // Set the flag to indicate the 'questions2' set
}
else{
   set=questions;
   flag=0;
...
nextButton.addEventListener('click', () => {
    ...
    // Loop through each option for anxiety
  optionsAnxiety.forEach(function (el) {
    ...
    if (el.classList.contains("active")) {
      ...
      if (i < set.length - 1) {
        i++;
        questionGenerator(i); // Generate the next question
      }
    }
...
// Function to generate a question based on the index 'i'
...
// Event listener for each option related to anxiety
optionsAnxiety.forEach(function (el, key) {
  el.addEventListener('click', function (e) {
    let value = e.target.textContent;

    // Assign scores based on the selected value
    if (value == "Usually") {
      set[i].score = 4;
    ...
    el.classList.toggle("active");  // Toggle 'active' class for the selected option
...
checkResult.addEventListener('click', () => {
  ...
  for (let i = 0; i < set.length; i++) {
    ...
    // Hide the 'Check Result' button and the question section
...
// Initialize the score for depression
function fetchScoreDepression(){
  ...
  // Hide the depression test section after collecting the score
...
// Functions to handle user choices and set scores/UI elements
function filler1() {
...
// Other similar functions (angry, nervous, sleepy) setting scores and UI based on user choices...
function angry() {
...
// Function to update score and UI based on user choice
function pop() {
  if (choice_checker == 2) {
...
// Function to manage visibility of question sets and the 'Check My Responses' button
